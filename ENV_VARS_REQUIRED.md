...
| `RATE_LIMIT_STRICT` | OPTIONAL (default in-memory fallback in production) | When Upstash Redis is unavailable, production enforces tier limits via **deterministic in-memory per-instance windows** — no blanket denial (the pre-2026-06-11 fail-closed default turned an Upstash brownout into a full platform outage where every route answered 429). Set `true` to select the in-memory fallback in every environment; `false` opts into the race-prone blob soft fallback (logged at startup). | _(unset)_ |
