#!/usr/bin/env node
/**
 * Setup script for the Compliance Analyzer Claude memory system.
 */
...
console.log('Compliance Analyzer Claude Memory System -- Setup');
console.log('=================================================\n');
...
try {
  ...
  if (existing.length === 0) {
    db.addSummary({
      ...
      content: [
        'Hawkeye Sterling V2 (Compliance Analyzer) -- UAE AML/CFT/CPF compliance suite.',
        'Stack: TypeScript/React frontend, Netlify Functions backend.',
        'Constants source of truth: src/domain/constants.ts',
        'Key thresholds: AED 55K (transaction), AED 60K (cross-border), 25% (UBO), 24h (freeze).',
        'Primary law: Federal Decree-Law No. 10 of 2025.',
        'NEVER cite Federal Decree-Law No. 20 of 2018 (deprecated).',
        'Quick scenarios: AML/CFT, KYC/KYS, PEP, TFS, Responsible Sourcing, SOF/SOW, UBO, ESG.',
      ].join('\n'),
      tokens: 100,
    });
    console.log('   Seeded initial L0 compliance context.');
  } else {
