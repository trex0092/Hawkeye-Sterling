...
if (-not $BASE_URL) { Write-Host "  ERROR: BASE_URL is not set." -ForegroundColor Red; exit 1 }
if (-not $ADMIN_TOKEN) { Write-Host "  ERROR: ADMIN_TOKEN is not set." -ForegroundColor Red; exit 1 }
if (-not $ASANA_TOKEN) { Write-Host "  ERROR: ASANA_TOKEN is not set." -ForegroundColor Red; exit 1 }
if (-not $ASANA_PROJECT_GID) { Write-Host "  ERROR: ASANA_PROJECT_GID is not set." -ForegroundColor Red; exit 1 }

$ts = [DateTimeOffset]::UtcNow.ToUnixTimeSeconds()
$smokeSubject = "Hawkeye Smoke Test $ts"
$smokeCaseId = "SMOKE-$ts"
...
try {
    $r = Invoke-WebRequest -Uri "https://app.asana.com/api/1.0/users/me" -Headers $asanaHeaders -UseBasicParsing
    if ($r.StatusCode -eq 200) {
        Write-Pass "Asana API reachable (HTTP 200)"
    } else {
        Write-Fail "Asana API returned HTTP $($r.StatusCode)"
    }
} catch {
...
try {
    $r = Invoke-WebRequest -Uri "$BASE_URL/api/status" -Headers $appHeaders -UseBasicParsing
    if ($r.StatusCode -eq 200) {
        Write-Pass "App /api/status returned HTTP 200"
    } else {
        Write-Fail "App /api/status returned HTTP $($r.StatusCode)"
    }
...
if ($taskGid) {
    try {
        $r = Invoke-WebRequest -Uri "https://app.asana.com/api/1.0/tasks/$taskGid" -Headers $asanaHeaders -UseBasicParsing
...
if ($taskGid) {
    try {
        $r = Invoke-WebRequest -Uri "https://app.asana.com/api/1.0/tasks/$taskGid/attachments" -Headers $asanaHeaders -UseBasicParsing
...
if ($taskGid) {
    try {
        $r = Invoke-WebRequest -Uri "https://app.asana.com/api/1.0/tasks/${taskGid}?opt_fields=projects" -Headers $asanaHeaders -UseBasicParsing
...
if ($cleanup -and $taskGid) {
    ...
    try {
        Invoke-WebRequest -Uri "https://app.asana.com/api/1.0/tasks/$taskGid" -Method DELETE -Headers $asanaHeaders -UseBasicParsing | Out-Null
