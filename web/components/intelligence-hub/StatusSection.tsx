...
interface BrainSoul {
  ...
  catalogue: { faculties: number; reasoningModes: number; metaCognition: number; skills: number };
}
...
interface SanctionsList { id: string; ageH: number | null; recordCount: number | null }
...
export function StatusSection() {
  ...
  const endpointHealth: "operational" | "degraded" | "down" | undefined =
    data?.status;
  useHubSignal("endpointHealth", endpointHealth, [endpointHealth]);
        ...
        <>
          ...
          {data.status === "operational" && data.externalStatus && data.externalStatus !== "operational" && (
            <div className="rounded-lg p-3 mb-4 flex items-start gap-3 bg-bg-panel border border-hair-2 text-ink-2">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-12 font-semibold text-ink-1">External dependency notice</div>
          ...
          {data.maintenance && data.maintenance.length > 0 && (
            <div className="rounded-lg p-4 mb-4 bg-blue-dim text-blue flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-13 font-semibold mb-1">Scheduled maintenance</div>
          ...
          <div className="bg-bg-panel border border-hair-2 rounded-lg p-6 mb-6">
            <div className="flex items-center gap-3 mb-4">
              <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded font-mono text-11 font-semibold ${STATUS_TONE[data.status]}`}>
              ...
              {data.cognitiveGrade && (
                <div className="ml-auto text-right">
                  ...
                  <div className="font-mono text-10 text-ink-3">{data.cognitiveGrade.score}/100</div>
                </div>
              )}
            </div>
            <div className="flex gap-8 text-12 text-ink-2 font-mono flex-wrap">
              <span>Session uptime: <span className="text-ink-0">{fmtUptime(data.uptimeSec)}</span></span>
          ...
          <StatusSection_inner title="Internal services">
            <div className="space-y-2">
              {data.checks.map((c) => <ServiceRow key={c.name} check={c} />)}
            </div>
          </StatusSection_inner>

          <StatusSection_inner title="Service dependency map">
            <ServiceDependencyMap checks={data.checks} externalChecks={data.externalChecks} />
          </StatusSection_inner>

          <StatusSection_inner title="External dependencies">
            <div className="space-y-2">
              {data.externalChecks.map((c) => <ServiceRow key={c.name} check={c} />)}
            </div>
          </StatusSection_inner>

          {data.configHealth && (
            <StatusSection_inner title="Environment config">
          ...
          <StatusSection_inner title="Sanctions-list freshness">
            <div className="bg-bg-panel border border-hair-2 rounded px-4 py-3 mb-2">
              <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
                <div className="flex items-center gap-3 flex-wrap">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded font-mono text-10 font-semibold ${STATUS_TONE[effectiveSanctionsStatus(data.sanctions)]}`}>
                    {effectiveSanctionsStatus(data.sanctions)}
                  </span>
                  <span className="text-13 text-ink-0 font-medium">sanctions-freshness</span>
                  <span className="text-11 text-ink-3 font-mono">
                    · {
              ...
              {data.sanctions.lists.length > 0 ? (
                ...
                <div className="text-11 font-mono text-ink-3">
                  No list data yet — sanctions cron hasn&apos;t run.
                </div>
              )}
            </div>
          </StatusSection_inner>

          <StatusSection_inner title="90-day uptime">
          ...
          <StatusSection_inner title="Session activity (last 5 min)">
            <SessionActivity
          ...
          <StatusSection_inner title="Incident history">
            {data.incidents.length === 0 ? (
              ...
              <div className="space-y-2">
                {data.incidents.map((i) => (
                  <div key={i.id} className="bg-bg-panel border border-hair-2 rounded px-4 py-3">
                    <div className="flex items-baseline justify-between gap-3">
                      <span className="text-13 text-ink-0 font-medium">{i.title}</span>
                      <span className={`font-mono text-10 uppercase ${
                        i.severity === "critical" ? "text-red" : i.severity === "major" ? "text-orange" : "text-amber"
                      }`}>{i.severity}</span>
                    </div>
                    ...
                    {i.affected.length > 0 && (
                      <div className="text-11 text-ink-3 mt-0.5">Affected: {i.affected.join(", ")}</div>
                    )}
                  </div>
                ))}
              </div>
          ...
          <StatusSection_inner title="Data-feed versions">
            <div className="bg-bg-panel border border-hair-2 rounded px-4 py-3 grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-1 text-11 font-mono">
              {[
                ["brain", data.feedVersions.brain],
                ["commit", data.feedVersions.commitSha],
                ["reviewed at", data.feedVersions.reviewedAt],
                ["AM categories", String(data.feedVersions.adverseMediaCategories)],
                ["AM keywords", String(data.feedVersions.adverseMediaKeywords)],
                ["known PEPs", String(data.feedVersions.knownPepEntries)],
              ].map(([label, value]) => (
          ...
          <StatusSection_inner title="Recent deploys">
              ...
              {data.deploys.map((d) => (
                <div key={d.id} className="px-4 py-2.5 flex items-center justify-between">
                  <div className="flex items-center gap-3 min-w-0">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded font-mono text-10 font-semibold ${
                      d.state === "success" ? "bg-green-dim text-green"
                      : d.state === "error" ? "bg-red-dim text-red"
                      : "bg-amber-dim text-amber"
                    }`}>
                      {d.state}
                    </span>
          ...
          <div className="mt-8 text-11 text-ink-3 flex flex-wrap gap-4">
            <span>JSON: <a href="/api/status" target="_blank" rel="noopener noreferrer" className="underline hover:text-ink-1">/api/status</a></span>
            <span>RSS feed: <a href="/api/status/feed" target="_blank" rel="noopener noreferrer" className="underline hover:text-ink-1">/api/status/feed</a></span>
          </div>
        </>
      )}
...
function SanctionsRefreshButton() {
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap mt-2">
      ...
      {state === "done" && <span className="text-10 text-green font-mono">{msg}</span>}
      {state === "error" && <span className="text-10 text-red font-mono">{msg}</span>}
      {state === "idle" && <span className="text-10 text-ink-3 font-mono">Invalidates the in-process cache — live data reloads on next screen.</span>}
    </div>
  );
}
...
function DependencyNode({ label, status, external = false }: { label: string; status?: SvcStatus; external?: boolean }) {
  const dot = status === "operational" ? "bg-green" : status === "degraded" ? "bg-amber" : status === "down" ? "bg-red" : "bg-ink-3";
...
function SessionActivity({ historyRef, checks, externalChecks, sanctionsName, sanctionsStatus: _ss }: {
  ...
  const allSvcs = [...checks.map((c) => c.name), ...externalChecks.map((c) => c.name), sanctionsName];
        ...
        {allSvcs.map((name) => {
          ...
          return (
            <div key={name} className="flex items-center gap-2">
              ...
              {errCount > 0 && <span className="text-10 font-mono text-amber">{errCount} non-ok</span>}
            </div>
          );
        })}
      </div>
...
const GRADE_COLOR: Record<CognitiveGrade["grade"], string> = {
  "A+": "text-green", "A": "text-green", "B": "text-amber", "C": "text-orange-500", "F": "text-red",
};

function BrainNarrativePanel({ narrative, compositeHash, grade, now }: {
  narrative: string; compositeHash: string; grade?: CognitiveGrade; now: string;
}) {
  return (
    <div className="mb-6">
      <div className="text-10.5 font-semibold uppercase tracking-wide-4 text-ink-2 mb-2">Brain assessment</div>
      <div className="bg-bg-panel border border-hair-2 rounded-lg px-5 py-4">
        ...
        <div className="mt-3 pt-3 border-t border-hair flex items-center justify-between">
          <span className="font-mono text-10 text-ink-3">seal: <span className="text-ink-2">{compositeHash}</span></span>
          {grade && (
...
function ThreatSurfacePanel({ surface }: { surface: ThreatSurface }) {
  ...
  return (
    <div className="mb-6">
      <div className="text-10.5 font-semibold uppercase tracking-wide-4 text-ink-2 mb-2">Compliance threat surface</div>
          ...
          {surface.impaired.map((e) => {
            ...
            return (
              <div key={`${e.complianceFunction}-${e.affectedService}`} className="flex items-center gap-3 flex-wrap">
                ...
                <span className="font-mono text-10 text-ink-3 flex-shrink-0">{e.affectedService} · {e.serviceStatus}</span>
              </div>
            );
          })}
        </div>
...
function BrainSoulPanel({ soul }: { soul: BrainSoul }) {
  ...
  return (
    <div className="mb-6">
      <div className="text-10.5 font-semibold uppercase tracking-wide-4 text-ink-2 mb-2">Brain · soul</div>
      <div className={`bg-bg-panel border ${tone.border} rounded-lg p-5`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-11 text-ink-2 font-mono">
                {directives} amplifier directives &nbsp;·&nbsp; amplifier {soul.amplifierVersion}
              </div>
            </div>
          </div>
        ...
        <div className="mt-4 pt-4 border-t border-hair grid grid-cols-1 sm:grid-cols-3 gap-x-6 gap-y-1.5 text-10.5 font-mono">
          {([
            ["charterHash", soul.charterHash],
            ["catalogueHash", soul.catalogueHash],
            ["compositeHash", soul.compositeHash],
          ] as const).map(([label, value]) => (
            <div key={label} className="flex justify-between gap-3">
              <span className="text-ink-3">{label}</span>
              <span className={value === "unavailable" || value === "missing" ? "text-red" : "text-ink-1"}>{value}</span>
            </div>
          ))}
        </div>
        {soul.status !== "intact" && (
...
function StatusSection_inner({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-6">
      <div className="text-10.5 font-semibold uppercase tracking-wide-4 text-ink-2 mb-2">{title}</div>
      {children}
    </div>
  );
}
...
function ServiceRow({ check }: { check: Check }) {
  const hasPercentiles = check.p50 !== undefined || check.p95 !== undefined || check.p99 !== undefined;
  return (
    <div className="bg-bg-panel border border-hair-2 rounded px-4 py-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3 min-w-0">
          <span className={`inline-flex items-center px-2 py-0.5 rounded font-mono text-10 font-semibold ${STATUS_TONE[check.status]}`}>
            {check.status}
          </span>
          <span className="text-13 text-ink-0 font-medium">{check.name}</span>
          {check.note && <span className="text-11 text-ink-3 font-mono truncate">· {check.note}</span>}
        </div>
...
function UptimeTimeline({ name, samples }: { name: string; samples: Check["status"][] }) {
  ...
  return (
    <div>
      ...
      <div className="flex gap-[2px] h-5">
        {samples.map((s, i) => (
          <div key={`bar-${i}`} className={`flex-1 rounded-sm ${BAR_TONE[s]}`} title={`Day -${samples.length - 1 - i}: ${s}`} />
        ))}
      </div>
    </div>
  );
