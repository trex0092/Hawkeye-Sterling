...
export function AnalyticsSection() {
  ...
  useHubSignal("fpRate", data ? Math.round(fpRate * 1000) / 10 : undefined, [fpRate, data]);
  ...
  return (
    <div className="print:bg-white">
      <ModuleHero
        eyebrow="Module · Analytics & KPI Dashboard"
        title="MLRO performance"
        titleEm="digest."
        intro={
          <>
      ...
      <div className="print:px-6 print:py-6">
                  ...
                  {predictRisk.interventions.map((inv, i) => {
                    ...
                    return (
                      <div key={i} className="bg-bg-1 rounded-lg p-3 space-y-2 border border-hair">
                        ...
                        <div className="inline-flex items-center px-2 py-0.5 rounded-full text-10 font-semibold bg-green-dim text-green">
                          {inv.expectedImpact}
                        </div>
                      </div>
                    );
        ...
        <AnalyticsSection_inner label="Headline metrics">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-6 print:gap-4">
            ...
            <Headline value={auditCount > 0 ? `${auditCount}` : "0"} caption="Audit entries logged" />
          </div>
        </AnalyticsSection_inner>

        <AnalyticsSection_inner label="Screening volume (last 12 weeks)">
          {weekly.every((v) => v === 0) ? (
        ...
        <AnalyticsSection_inner label="Findings breakdown">
          <FindingsBars rows={findings} />
        </AnalyticsSection_inner>

        <AnalyticsSection_inner label="Regulatory filings · month to date">
        ...
        <AnalyticsSection_inner label="Compliance posture">
        ...
        <AnalyticsSection_inner label="Monitoring coverage">
          {data ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 print:gap-3">
              <Metric label="Enrolled in ongoing screening" value={String(data.monitoring.enrolledSubjects)} />
              <Metric label="Scheduled for rerun" value={String(data.monitoring.scheduledSubjects)} />
              <Metric label="Analyst verdicts (24h)" value={String(data.quality.verdictsLast24h)} />
            </div>
          ) : (
            <Empty>Loading…</Empty>
          )}
        ...
        {data && (
          <AnalyticsSection_inner label={`DPMS KPI catalogue · ${data.kpis.defined} indicators`}>
...
function AnalyticsSection_inner({
  label,
  children,
}: {
  label: string;
