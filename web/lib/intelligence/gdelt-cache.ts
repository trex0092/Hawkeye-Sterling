...
async function liveFetch(subjectName: string, customQuery?: string): Promise<{ articles: GdeltArticle[]; serviceError: boolean; breakerOpen?: boolean }> {
  // Short-circuit when the breaker is OPEN and the cooldown hasn't yet
  // elapsed. Returning { serviceError: true } steers fetchGdeltCached()
  // into the stale-Redis recovery path without any upstream network call.
