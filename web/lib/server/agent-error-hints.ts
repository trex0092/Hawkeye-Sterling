...
const PATTERNS: ErrorPattern[] = [
  {
    ...
    describe: (raw) => {
      ...
      return {
        ...
        hint: [
          `The Data Analyst agent cannot reach ${serverName} because no credential is stored in the Claude Console vault for its server URL.`,
          ``,
          `Resolution (Claude Console admin):`,
          `  1. Open https://console.anthropic.com`,
          `  2. Navigate to Agents → Data Analyst → Configuration`,
          `  3. Open the MCP servers panel and locate the ${serverName} entry`,
          `  4. Add or update the credential in the linked vault entry`,
          `  5. Confirm the server URL in the agent configuration exactly matches the vault URL (whitespace and trailing slashes count)`,
          `  6. Save and retry the request`,
          ``,
          `Until this is resolved the Data Analyst tab cannot return results — every request will fail at session-init time.`,
        ].join("\n"),
      };
    },
  },
