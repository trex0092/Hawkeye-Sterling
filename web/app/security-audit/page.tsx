...
const CHECKLIST = [
  {
    ...
    items: [
      {
        id: "api1",
        text: "Anthropic API key is NOT in any frontend JS file (screening-app.js, tfs-engine.js, app.html)",
      },
      {
        id: "api2",
        text: "Asana API token stored only in Netlify environment variables, not hardcoded",
      },
      {
        id: "api3",
        text: "All api.anthropic.com calls go through netlify/functions, never direct from browser",
      },
      {
        id: "api4",
        text: "Environment variables verified in Netlify dashboard: Site Settings → Environment Variables",
      },
    ],
  },
  {
    ...
    items: [
      {
        id: "csp1",
        text: "Remove 'unsafe-inline' from style-src — use nonces or hashes instead",
      },
      {
        id: "csp2",
        text: "Change X-Frame-Options from SAMEORIGIN to DENY (align with frame-ancestors 'none')",
      },
      {
        id: "csp3",
        text: "Add report-uri or report-to directive to CSP for violation monitoring",
      },
      { id: "csp4", text: "Add upgrade-insecure-requests to CSP" },
      {
        id: "csp5",
        text: "Remove connect-src api.anthropic.com if API calls move fully to serverless",
      },
    ],
  },
  {
    ...
    items: [
      {
        id: "fn1",
        text: "All functions validate and sanitize incoming request bodies before processing",
      },
      {
        id: "fn2",
        text: "Functions return generic error messages (no stack traces or internal paths to client)",
      },
      {
        id: "fn3",
        text: "Rate limiting implemented on screening endpoints to prevent abuse",
      },
      {
        id: "fn4",
        text: "CORS origins explicitly restricted (not wildcard *)",
      },
    ],
  },
  {
    ...
    items: [
      {
        id: "auth1",
        text: "Screening console (app.html) is protected — not publicly accessible without authentication",
      },
      {
        id: "auth2",
        text: "Session tokens expire appropriately and are invalidated on logout",
      },
      {
        id: "auth3",
        text: "No sensitive compliance data (PII, screening results) stored in localStorage or sessionStorage",
      },
      {
        id: "auth4",
        text: "Role-based access: MLRO functions require elevated permissions",
      },
    ],
  },
  {
    ...
    items: [
      {
        id: "aud1",
        text: "Screening results are immutable once written — no client-side tampering possible",
      },
      {
        id: "aud2",
        text: "All API calls to Asana are server-side only (audit trail cannot be bypassed from browser)",
      },
      {
        id: "aud3",
        text: "Timestamps on compliance events use server time, not client time",
      },
    ],
  },
  {
    ...
    items: [
      {
        id: "dep1",
        text: "npm audit run on root package.json — zero high/critical CVEs",
      },
      { id: "dep2", text: "npm audit run on web/ subdirectory" },
      {
        id: "dep3",
        text: "Snyk or Dependabot enabled for automated CVE alerts",
      },
      {
        id: "dep4",
        text: "gen-weaponized-brain.cjs script reviewed — does not embed secrets in output",
      },
    ],
  },
  {
    ...
    items: [
      {
        id: "net1",
        text: "Cross-Origin-Opener-Policy (COOP) header added: same-origin",
      },
      {
        id: "net2",
        text: "Cross-Origin-Resource-Policy (CORP) header added: same-origin",
      },
      {
        id: "net3",
        text: "Deploy previews restricted — not publicly accessible to unauthenticated users",
      },
      {
        id: "net4",
        text: "Netlify access logs enabled and reviewed periodically",
      },
    ],
  },
];
...
> = {
  CRITICAL: {
    badge: "bg-red-dim text-red",
    border: "border-l-red",
    dot: "bg-red",
  },
  HIGH: {
    badge: "bg-orange-dim text-orange",
...
export default function SecurityAuditPage() {
  const [tab, setTab] = useState<"analyser" | "tools" | "checklist">(
    "analyser"
  );
  ...
  return (
    <ModuleLayout
      asanaModule="security-audit"
      asanaLabel="Security audit"
      sidebarExtra={
      ...
      <div className="px-4 md:px-8 py-6 max-w-3xl">
        ...
        {tab === "analyser" && (
          <div className="space-y-4">
            <p className="text-sm text-ink-2 leading-relaxed">
              ...
              <span className="text-brand font-mono text-[12px]">
                screening-app.js
              </span>
              ,{" "}
              <span className="text-brand font-mono text-[12px]">
              ...
              , or a Netlify function. Claude analyses it server-side for
              ...
              <div ref={resultRef} className="space-y-4 pt-2">
                ...
                <div className="flex gap-5 items-start p-5 bg-bg-1 border border-hair-2">
                  <div className="text-center shrink-0">
                    <div
                      className={`font-mono text-4xl font-semibold ${scoreColor}`}
                    >
                      {result.score}
                    </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-ink-0 leading-relaxed mb-2">
                      {result.summary}
                    </p>
                ...
                <p className="font-mono text-[10px] uppercase tracking-widest text-ink-3">
                  {result.findings?.length ?? 0} finding
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2.5 flex-wrap">
                        <span
                          className={`font-mono text-[9px] uppercase tracking-widest px-2 py-0.5 ${sev.badge}`}
                        >
                          {f.severity}
                        </span>
        ...
        {tab === "tools" && (
          <div className="space-y-5">
            <p className="text-sm text-ink-2 leading-relaxed">
              Four free tools you can run on the Hawkeye Sterling repository
              today.
            </p>
            ...
            {(() => {
              ...
              return (
                <div className={`border ${t.borderClass} p-5 space-y-4`}>
                  <div>
                    <p
                      className={`font-mono text-[11px] uppercase tracking-widest mb-0.5 ${t.accentClass}`}
                    >
                      {t.name}
                    </p>
                    <p className="text-sm text-ink-2 leading-relaxed">
                  ...
                  <div
                    className={`p-3 border-l-2 ${t.borderClass} bg-bg-1 font-mono text-[11px] leading-relaxed ${t.accentClass}`}
                  >
