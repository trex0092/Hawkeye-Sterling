...
function buildReport(m: AsanaModuleTask, date: string, input: ReportInput): string {
  ...
  const lines: string[] = [
    ...
    `• Every action written to the immutable, hash-linked, append-only audit chain (FDL 10/2025 Art.24).`,
    ...
    `• AI-assisted outputs subject to human-in-the-loop review on adverse dispositions (FDL 10/2025 Art.18).`,
    ``,
    `§5 · FINDINGS (last 24h)`,
    input.findings,
  ];
  ...
  lines.push(
    ...
    `• AI governance: model governance, explainability and human oversight maintained per FDL 10/2025 Art.18/24 (ISO/IEC 42001; EU AI Act-aligned).`,
    ``,
    ...
    `• Retention: ${m.retention} (FDL 10/2025 Art.24; FDL record-keeping & Cabinet 10/2019).`,
