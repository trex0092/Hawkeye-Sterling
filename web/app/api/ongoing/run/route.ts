...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  // Process all subjects concurrently — sequential awaits would push wall-clock
  // time past Netlify's 10s cap for portfolios > ~30 subjects.
    ...
    subjects.map(async (s) => {
      try {
        ...
        return {
          ...
          topScore: 0,
          severity: "error",
          newHits: [] as Array<{ listId: string; listRef: string; candidateName: string }>,
          webhook: {
            delivered: false,
