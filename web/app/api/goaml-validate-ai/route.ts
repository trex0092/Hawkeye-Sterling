...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  try {
    ...
    const res = await client.messages.create({
        ...
        max_tokens: 700,
        system:
          "You are a UAE FIU goAML submission quality reviewer. Check whether this STR/SAR narrative meets all requirements under Federal Decree-Law No. 10 of 2025 Art.26, FATF R.20, and goAML submission standards. A valid narrative must answer Who/What/When/Where/Why, document the specific suspicion, avoid tipping-off language, and include enough detail for the FIU to act. Return ONLY valid JSON — no markdown fences, no commentary.",
    ...
    if (!Array.isArray(result.tippingOffFlags)) result.tippingOffFlags = [];
    if (typeof result.tippingOffRisk !== "boolean") result.tippingOffRisk = result.tippingOffFlags.length > 0;
    if (typeof result.grade !== "string" || !["PASS", "CONDITIONAL_PASS", "FAIL"].includes(result.grade)) {
      result.grade = result.score >= 75 ? "PASS" : result.score >= 50 ? "CONDITIONAL_PASS" : "FAIL";
    }
    if (typeof result.fiuReadiness !== "string") result.fiuReadiness = "";
  } catch (err) {
