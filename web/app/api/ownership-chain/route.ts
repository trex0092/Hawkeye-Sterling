...
export async function POST(req: NextRequest) {
  ...
  // enforce()/audit-chain back-ends) so the route returns a 500 JSON
  // instead of crashing the Netlify Lambda — which would render the
  // "This function has crashed" page in the UBO Walker tab iframe.
  try {
  ...
  void writeAuditChainEntry(
    ...
    console.error('[hawkeye] ownership-chain handler exception:', message);
    return NextResponse.json(
