...
function ewraHistoryKey(sector: string, jurisdiction: string): string {
  // Normalise to avoid key collisions on different casing / spacing
...
async function loadPreviousEwraSnapshot(
  sector: string,
  jurisdiction: string,
...
async function saveEwraSnapshot(snapshot: StoredEwraSnapshot): Promise<void> {
  try {
...
export interface EwraResult {
  ...
  fatfRiskMatrix: {
    ...
    appliedFactors: Array<{ factor: string; category: string; baseScore: number; ratingLabel: string }>;
  };
  sectorModifiers: {
    totalModifier: number;
    applied: Array<{ sector: string; modifier: number; regulatoryBasis: string }>;
  };
...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  // 1. FATF risk factor matrix category scores
...
The overallScore should reflect the FATF category scores and sector modifier above.`,
    ...
    // Apply sector modifier to inherent score (capped at 100)
