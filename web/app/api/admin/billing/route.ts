...
function bucketKey(tenant: string, month: string): string {
  return `usage/${tenant}/${month}.json`;
}

/** Increment a usage counter for the supplied tenant + metric. Best-
 *  effort — wraps Blob errors so calling routes never crash on quota IO. */
