...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  writeAuditEvent("compliance_assistant", "screening.false-positive-assess", screenedName);
  ...
  try {
    ...
    writeAuditEvent("compliance_assistant", "screening.false-positive-assess.error", `${screenedName} — ${msg}`);
    return NextResponse.json({ ok: false, error: "false-positive temporarily unavailable - please retry." }, { status: 503 , headers: gate.headers });
  }
}
