...
export async function POST(req: Request) {
  ...
  if (!apiKey) return NextResponse.json({ ok: false, error: "freeze-seizure temporarily unavailable - please retry." }, { status: 503 });
...
- Additional Context: ${body.context}`,
    ...
    if (!jsonMatch) return NextResponse.json({ ok: false, error: "freeze-seizure temporarily unavailable - please retry." }, { status: 503 });

    const parsed = JSON.parse(jsonMatch[0]) as FreezeSeizureResult;
    return NextResponse.json({ ok: true, ...parsed });
  } catch {
    return NextResponse.json({ ok: false, error: "freeze-seizure temporarily unavailable - please retry." }, { status: 503 });
  }
}
