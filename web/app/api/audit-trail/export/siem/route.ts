// GET /api/audit-trail/export/siem
//
// Exports audit trail entries in Splunk HEC-compatible NDJSON format
// (Splunk/Elastic Common Schema). Each line is a Splunk HEC event envelope:
//
//   {"time": <epoch_seconds>, "source": "hawkeye-sterling",
//    "sourcetype": "aml:audit", "event": { ...original entry... }}
//
// Query params:
...
//
// Response:
//   application/x-ndjson — one Splunk HEC JSON object per line
//   Content-Disposition: attachment; filename="siem-export-YYYY-MM-DD.ndjson"
//   x-siem-format: splunk-hec
//
...
export async function GET(req: Request): Promise<Response> {
  ...
  try {
    ...
    console.error("[audit-trail/export/siem] chain read failed:", err instanceof Error ? err.message : String(err));
    return NextResponse.json(
  ...
  // Transform each entry into a Splunk HEC event envelope.
  const lines = filtered
    .map((entry): string => {
      const epochSeconds = new Date(entry.at).getTime() / 1000;
      const hecEvent: SplunkHecEvent = {
        time: epochSeconds,
        source: "hawkeye-sterling",
        sourcetype: "aml:audit",
        event: entry,
      };
  ...
  return new Response(lines, {
    ...
    headers: {
      ...
      "content-disposition": `attachment; filename="siem-export-${dateStamp}.ndjson"`,
      "x-siem-format": "splunk-hec",
